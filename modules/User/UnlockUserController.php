...
/**
 * POST /users/{id}/unlock - dung chung permission user.lock voi LockUserController (Owner
 * Decision CMS-037 - Lock/Unlock cung 1 capability). 404 neu user khong thuoc tenant hien tai.
 */
final class UnlockUserController
{
    ...
    public function handle(Request $request): Response
    {
        ...
        $this->database->statement('UPDATE users SET status = ? WHERE id = ?', ['active', $userId]);

        return Response::json([
            'success' => true,
            'data' => null,
            'message' => 'Da mo khoa user.',
            'errors' => [],
        ]);
    }
}
