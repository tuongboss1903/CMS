<?php
/**
 * Bo icon SVG inline dung chung toan Admin (khong phu thuoc CDN/font-icon, khop nguyen tac
 * "self-contained" cua theme engine). Goi qua $this->include('admin.partials.icon', ['name' =>
 * 'dashboard', 'class' => 'icon']) - $name khong hop le se khong render gi (khong throw, tranh vo
 * trang neu ai do go sai ten trong luc them moi UI). Stroke-based (Heroicons outline style),
 * 20x20 mac dinh, dung 'currentColor' de tu doi mau theo CSS (nut/badge/sidebar).
 */
$icons = [
...
?><svg class="<?= $class ?>" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><?= $this->raw($icons[$name]) ?></svg>
