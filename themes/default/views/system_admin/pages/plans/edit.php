...
<?php endif; ?>
<form method="POST" action="/system-admin/plans/<?= $this->e((string) $plan['id']) ?>" class="card card--spacious" style="max-width: 480px;">
    <input type="hidden" name="_token" value="<?= $this->e($csrf_token ?? '') ?>">
    <div class="field-grid-2">
    <div class="field">
        <label for="key">Key <span class="req">*</span></label>
        <input type="text" id="key" name="key" value="<?= $this->e((string) ($old['key'] ?? '')) ?>" required<?= empty($errors['key']) ? '' : ' aria-invalid="true"' ?>>
