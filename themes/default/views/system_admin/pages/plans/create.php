...
<?php endif; ?>
<form method="POST" action="/system-admin/plans" class="card" style="max-width: 480px;">
    <input type="hidden" name="_token" value="<?= $this->e($csrf_token ?? '') ?>">
    <div class="field">
        <label for="key">Key</label>
        <input type="text" id="key" name="key" value="<?= $this->e((string) ($old['key'] ?? '')) ?>" placeholder="basic" autofocus>
    </div>
    <div class="field">
        <label for="name">Ten goi</label>
        <input type="text" id="name" name="name" value="<?= $this->e((string) ($old['name'] ?? '')) ?>" placeholder="Co ban">
    </div>
    <div class="field">
    ...
    <button type="submit" class="btn btn-primary">Tao Goi</button>
