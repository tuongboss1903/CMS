...
> **Trạng thái Standby (đánh dấu chính thức, Phase 6)**: qua toàn bộ vòng đời dự án (CMS-001 → Phase 6), **không Module nghiệp vụ nào gọi `Cache`** — vẫn là dead capability, đã viết sẵn/test sẵn (không lỗi) nhưng chưa có consumer thật. Giữ nguyên trong codebase (không xóa) — sẵn sàng dùng ngay khi có nhu cầu thật (ví dụ cache kết quả `SiteSettingsManager::get()` nếu volume truy vấn tăng).
...
> **Trạng thái Standby (đánh dấu chính thức, Phase 6)**: cùng tình trạng với `Cache` — không Module nghiệp vụ nào gọi `Hook`/đăng ký action-filter thật. `PluginManager` (mục 3.9+) có điểm nối (`onError()`) nhưng chưa có Plugin thật nào tồn tại để kiểm chứng luồng dùng chung.
...
**Role Model — ý nghĩa `roles.tenant_id`** (xác nhận qua đọc trực tiếp migration CMS-028; ý nghĩa suy ra từ 2 nguồn khớp nhau: comment `database-design.md` gốc + cách `CreateUserController` CMS-037 đã dùng câu query `WHERE tenant_id IS NULL OR tenant_id = ?`). **Đính chính (Phase 6)**: ghi chú trước đây ở dòng này nói migration "không có cột `is_system`" — **sai**, đọc lại trực tiếp `database/migrations/2026_08_01_000004_create_roles_table.php:19` xác nhận cột `is_system BOOLEAN NOT NULL DEFAULT 0` **tồn tại thật**. Cột này **chính thức deprecated**: tồn tại trong schema nhưng không Controller nào (JSON lẫn Admin) đọc/ghi qua toàn bộ vòng đời dự án — `tenant_id IS NULL` mới là nguồn sự thật duy nhất cho "System Role". Owner Decision Phase 6: **giữ nguyên cột** (không migration `DROP COLUMN` — hành động khó đảo ngược trên dữ liệu thật, chưa có lợi ích rõ ràng từ việc xóa), chỉ đánh dấu chính thức deprecated tại đây:
...
### 3.41. SEO Meta Settings Admin UI — mở rộng `modules/Admin/` — v0.0.55 (hoàn tất PHASE 3)

**Architecture**: `modules/Admin/` mở rộng thêm 3 Controller — **không thay đổi `modules/Seo/*` (API JSON)**, đúng tiền lệ Page/Media/Menu Admin UI (mục 3.38/3.39/3.40). Đây là module cuối của PHASE 3 — sidebar Admin không còn mục "Soon" nào.

**Phạm vi được thu hẹp qua `AskUserQuestion`**: yêu cầu ban đầu của Owner nhắc tới "Global SEO Settings" (Site Title format, Meta Description mặc định, OG Image mặc định, Robots.txt/Sitemap toggle) — khảo sát xác nhận tính năng này **hoàn toàn chưa tồn tại**: không có cột nào trên `sites`, không bảng `site_settings`, và `/sitemap.xml`/`/robots.txt` đã bị hoãn tường minh từ CMS-043 và Public Website Polish (mục 3.37) do rủi ro route collision với `GET /{slug}`. Owner xác nhận giữ đúng phạm vi hiện có của `modules/Seo/*` (chỉ SEO Meta theo từng Page) — Global Settings ghi nhận là khoảng trống, không triển khai trong module này.

**Controllers** (`modules/Admin/Seo*Controller.php`, copy logic từ `Modules\Seo\*Controller`): `SeoListController` (LEFT JOIN `seo_meta` để hiển thị badge Đã/Chưa cấu hình, không N+1), `SeoShowEditController`, `SeoUpdateController` (copy nguyên logic upsert từ `UpdateSeoMetaController` — SELECT rồi rẽ nhánh INSERT/UPDATE, không transaction, đúng lý do gốc CMS-043). Khác JSON API: `entity_type` cố định `'page'` trong Controller (không nhận qua route param) — Admin UI chỉ phục vụ Page.

**Routes** (mở rộng `modules/Admin/routes.php`):
```
GET  /admin/seo
GET  /admin/seo/pages/{id}
POST /admin/seo/pages/{id}
```

**`schema_data` qua form HTML**: JSON API gốc nhận `array` (JSON body), không có cách nhập mảng lồng nhau qua form text truyền thống — form dùng field mới `schema_data_json` (textarea JSON thô, không có trong JSON API gốc), `SeoUpdateController` tự `json_decode` trước khi áp dụng logic upsert; JSON không hợp lệ → silent-redirect, không lưu (cùng mẫu Media/Menu).

**Bug tự phát hiện và sửa trong lúc code**: cùng bug pattern `Validator`'s `nullable` không bỏ qua `''` đã gặp ở Menu Builder (mục 3.40) — `<select>` OG Image "-- Không có --" gửi `og_image_id=''`, luôn bị từ chối trước khi chạm logic xử lý. Sửa bằng chuẩn hóa `''` → `null` trước `validate()`.

**Permissions**: tái sử dụng nguyên `seo.view/seo.update` đã có từ CMS-043 (`bin/bootstrap.php`) — không thêm permission mới, không migration mới.

**Technical Debt ghi nhận**: (1) `modules/Admin/Seo*Controller.php` trùng lặp logic với `modules/Seo/*` (JSON) — cùng loại Technical Debt đã ghi nhận ở mục 3.38/3.39/3.40; (2) "Global SEO Settings" (Site Title format/Meta Description mặc định/OG Image mặc định/Robots.txt/Sitemap toggle) vẫn là khoảng trống — chưa có bảng/Module nào, cần Architecture Analysis riêng nếu triển khai trong tương lai. **(Đã giải quyết ở mục 3.42 — Phase 4, v0.0.56).**

**PHASE 3 hoàn tất**: 4 module Content (Pages, Media, Menu, SEO) đều đã có Admin UI đầy đủ (mục 3.38–3.41), tổng cộng 27 Controller Admin mới, sidebar không còn badge "Soon" nào.

### 3.42. Service Layer & Global Settings Management — `modules/Settings/*` — v0.0.56 (PHASE 4)

> **Đánh số**: mục này lẽ ra được đề xuất là "3.41" nhưng số đó đã dùng cho SEO Meta Settings Admin UI (v0.0.55, xem mục 3.41) — đánh số lại thành 3.42 để tránh trùng lặp, giữ đúng tiền lệ xử lý discrepancy đánh số đã áp dụng nhiều lần trước đây (CMS-032, CMS-046).

**Bối cảnh**: giải quyết 2 khoảng trống đã ghi nhận nhiều lần — (1) "Global SEO Settings" (flag từ mục 3.41), (2) Route Collision `/sitemap.xml`/`/robots.txt` với `GET /{slug}` (đã hoãn từ CMS-043 và mục 3.37 — "cách khắc phục duy nhất đòi hỏi chạm `modules/Public/module.json`, đã khóa").

**`SiteSettingsManager` (`modules/Settings/SiteSettingsManager.php`) — Service Layer đầu tiên của dự án**: mọi Module trước đó (Page/Media/Menu/Seo) đều theo nguyên tắc "Controller gọi `Database` trực tiếp, không Service/Repository". Ngoại lệ này được chấp nhận (Owner Decision, bỏ qua Architecture Analysis hình thức cho riêng quyết định này) vì `SiteSettingsManager` được tiêu thụ bởi **≥4 điểm độc lập không cùng 1 Controller sở hữu**: `SitemapController`, `RobotsController`, `modules/Public/{HomeController,PublicPageController}.php` (layout injection), `modules/Admin/SettingUpdateController.php` — khác hẳn pattern "1 Controller/1 nghiệp vụ" của mọi Module trước. API: `get(): array` (đọc `site_settings` theo tenant hiện tại qua `TenantManager`, trả default array nếu chưa có bản ghi), `set(array $fields): void` (upsert, cùng pattern SELECT-rồi-rẽ-nhánh-INSERT/UPDATE như `seo_meta`/CMS-043). **Cache runtime**: mảng PHP trong thuộc tính `private array $cache`, khóa theo `tenant_id`, sống trong đúng 1 request/1 Container instance — **không phải** `core/Cache.php` (Cache lớp core vẫn là dead capability, chưa Module nào dùng).

**Route Collision Resolution Pattern qua `module.json` dependencies**: `Router::match()` (`core/Router.php`) là **first-match-wins theo thứ tự đăng ký** (`foreach ($this->routes as $route)`, không ưu tiên độ cụ thể route). `ModuleManager::resolveLoadOrder()` (`core/ModuleManager.php`) dùng topological sort theo `dependencies` trong `module.json` để quyết định thứ tự `require routes.php` của từng Module — đây là **cách duy nhất** kiểm soát thứ tự đăng ký route giữa các Module khác nhau trong kiến trúc hiện tại (không có `priority`/`order` tường minh nào khác). Giải pháp: `modules/Public/module.json` thêm `"settings"` vào `dependencies` → `Settings` (chứa `GET /sitemap.xml`, `GET /robots.txt`) luôn boot **trước** `Public` (chứa `GET /{slug}` catch-all) → 2 route tĩnh luôn match trước khi Router thử khớp `{slug}`. **Không sửa `core/Router.php`/`core/ModuleManager.php`** — thuần dùng cơ chế dependency đã có sẵn từ CMS-034.

**`SitemapController`/`RobotsController`**: Public, không `Authorization::can()`. Sitemap chỉ liệt kê `pages.status='published' AND deleted_at IS NULL` của tenant hiện tại, `<loc>` build từ `https://` + `Request::getHost()` (giới hạn đã biết: `Request` chưa có `isSecure()`/`scheme()`, mặc định HTTPS). Robots.txt trả `robots_txt_custom` nếu Admin đã nhập, ngược lại mặc định an toàn (`Allow: /` + trỏ `Sitemap:`).

**Public Layout Integration — phạm vi thu hẹp có chủ đích** (Owner Decision qua `AskUserQuestion`): chỉ inject `site_settings['site_name']` vào logo header (`themes/default/views/layouts/main.php`), fallback "CMS.Demo" khi chưa cấu hình. **Không** tự sinh `default_meta_description`/OG fallback khi Page chưa có `seo_meta` riêng (sẽ đảo ngược "Option A: không tự sinh mặc định" đã chốt ở mục 3.37); **không** render `favicon` thật (chưa có route Public phục vụ file Media, giới hạn từ mục 3.37 chưa giải quyết).

**SEO Meta mở rộng** (`og_title`, `og_description`, `is_index`, `is_follow` — migration `2026_08_08_000001_alter_seo_meta_add_og_robots_fields.php`, 4 `ALTER TABLE ADD COLUMN` riêng lẻ để tương thích SQLite/MySQL): sửa `modules/Seo/UpdateSeoMetaController.php` (**lần đầu sửa file gốc JSON API Module** kể từ khi có tiền lệ Admin UI copy-logic, vì đây là mở rộng schema thật chứ không phải tạo Admin UI mới) và `modules/Admin/SeoUpdateController.php` + view. **Chỉ lưu DB `is_index`/`is_follow`, chưa render `<meta name="robots">` ra Public** (Owner Decision, tách biệt khỏi Public Layout Integration ở trên).

**Fix sau PHPUnit thật**: `ModuleSettingsIntegrationTest` FAIL 3 test (`NOT NULL constraint failed: pages.created_by`) — lỗi ở helper `seedPage()` của chính test mới viết (thiếu cột `created_by` khi INSERT), không phải lỗi Controller/Migration. Sửa: thêm `seedUser()` + bổ sung `created_by`.

**Permissions**: `settings.view`/`settings.update` mới (`bin/bootstrap.php`).

**Technical Debt ghi nhận**: (1) `SiteSettingsManager` là ngoại lệ Service Layer duy nhất — cần theo dõi nếu có thêm nhu cầu tương tự để quyết định có chính thức hóa pattern Service hay không; (2) Public Media Serve Route, `<meta name="robots">` render, Favicon thật, Breadcrumb, Search vẫn là khoảng trống — xem `TODO.md` Phase 5. **(Đã giải quyết ở mục 3.43 — Phase 5, v0.0.57).**

### 3.43. Public Engine Polish & Public Media Delivery — v0.0.57 (PHASE 5)

**Bối cảnh**: đóng 4 khoảng trống ghi nhận ở mục 3.42 — Public Media Serve Route, `<meta name="robots">` render thật, Favicon, Breadcrumb, Search.

**`MediaServeController` (`modules/Media/MediaServeController.php`, `GET /media/{filename}`) — sửa đổi bảo mật quan trọng so với đề xuất ban đầu**: đề xuất gốc `GET /media/{tenant_id}/{filename}` (nhận `tenant_id` từ URL) bị phát hiện là **IDOR** qua Architecture Analysis — toàn bộ hệ thống resolve tenant duy nhất qua domain (`TenantResolverMiddleware`/`TenantManager::id()`), không nơi nào khác tin tưởng `tenant_id` từ input. Thiết kế cuối: `GET /media/{filename}`, `tenant_id` luôn lấy từ `TenantManager::id()`. `{filename}` khớp theo **`media.path`** (tên file vật lý duy nhất do `uniqid()` sinh lúc Upload — `UploadMediaController`), **không phải** `media.file_name` (tên gốc người dùng, không duy nhất trong 1 tenant — phát hiện bug thứ 2 trong lúc code, đã sửa trước khi giao test). Đường dẫn vật lý luôn đọc từ `media.path` trong DB — không bao giờ nối trực tiếp `{filename}` vào filesystem path (chặn Path Traversal). Headers: `Cache-Control`/`ETag`/`Last-Modified`; **không** `304 Not Modified` thật (Owner Decision MVP — luôn trả `200`).

**`BreadcrumbBuilder` (`modules/Public/BreadcrumbBuilder.php`)**: walk ngược `pages.parent_id` từ trang hiện tại lên gốc, `MAX_DEPTH=20` (chặn vòng lặp vô hạn nếu dữ liệu `parent_id` hỏng thành chu trình — không có ràng buộc DB nào chống việc này). **Ngoại lệ thứ 2 tách class riêng** (sau `SiteSettingsManager` ở mục 3.42) thay vì trùng lặp method trong `HomeController`/`PublicPageController` như `fetchSeoMeta()`/`fetchNavigation()` — lý do: logic phức tạp hơn (vòng lặp, giới hạn độ sâu) khiến trùng lặp tốn kém hơn giá trị nhận được. **Không phải Service nghiệp vụ** (không ghi dữ liệu, không business rule) — chỉ hàm dùng chung thuần.

**`SearchController` (`modules/Public/SearchController.php`, `GET /search?q=`)**: `LIKE` trên `title`/`content` (bind tham số qua prepared statement — chặn SQL Injection), `status='published' AND deleted_at IS NULL`, scoped tenant hiện tại, `LIMIT 50` (hằng số PHP nối trực tiếp vào SQL — an toàn vì không phải input, không phải giới hạn kỹ thuật của bind parameter cho LIMIT). View `pages/search.php` escape `$query`/`$result['title']` qua `$this->e()` — không `$this->raw()` cho bất kỳ phần nào chứa input người dùng.

**Public SEO Header Integration** (`themes/default/views/layouts/main.php`): `<meta name="robots" content="{index|noindex},{follow|nofollow}">` dựa `seo_meta.is_index`/`is_follow` (mặc định `index,follow` khi chưa có `seo_meta`, khớp default cột DB); `<link rel="icon">` từ `site_settings.favicon_id`; `og:image` fallback `seo_meta.og_image_id` → `site_settings.default_og_image_id` — cả hai resolve qua `resolveMediaUrl()` (method riêng trong `HomeController`/`PublicPageController`, trùng lặp có chủ đích) trỏ tới `MediaServeController` mới.

**Route registration order**: `modules/Public/module.json` thêm `"media"` vào `dependencies` — đúng **Route Collision Resolution Pattern** đã thiết lập ở mục 3.42 (Module chứa route tĩnh phải boot trước Module chứa `GET /{slug}` catch-all). `GET /search` (route tĩnh trong chính `modules/Public/routes.php`) đăng ký trước `GET /{slug}` trong cùng file — thứ tự trong file cũng quyết định thứ tự global (`Router::match()` first-match-wins).

**Root Cause Analysis — trường hợp bác bỏ RCA đề xuất sai từ Owner**: sau PHPUnit thật, 2 test FAIL tại `PublicSearchTest`. Owner đề xuất sửa SQL trong `SearchController.php` (thêm `deleted_at IS NULL`/`LIMIT 50`) — xác minh trực tiếp source cho thấy **cả 2 đã có sẵn đúng từ đầu**, không phải nguyên nhân. Nguyên nhân thật: view `pages/search.php` luôn echo lại chính từ khóa tìm kiếm (`<p>Query: {q}</p>`), khiến 2 assertion trên chuỗi thô trong test bị nhiễu (đếm trùng khi `LIMIT 50` test dùng `substr_count`; match giả khi test "loại trừ trang đã xóa" tìm đúng chuỗi query). Sửa 2 assertion dùng tiền tố markup `<p>{title}</p>` thay vì chuỗi thô — **không đụng `SearchController.php`**. Đây là minh chứng cho nguyên tắc "không sửa mù theo RCA đề xuất mà chưa tự xác minh source" đã giữ xuyên suốt dự án.

**Permissions**: không permission mới (route Public, không `Authorization::can()`).

**Technical Debt ghi nhận**: (1) `BreadcrumbBuilder` là ngoại lệ tách-class thứ 2 — cùng câu hỏi để mở về việc chính thức hóa pattern Service/Helper dùng chung; (2) `304 Not Modified` thật chưa triển khai (Media Serve Route luôn trả `200` dù có cache header); (3) `Request` vẫn chưa có `isSecure()`/`scheme()` — `MediaServeController` không cần (không build URL tuyệt đối), nhưng `SitemapController`/`RobotsController` (mục 3.42) vẫn còn giới hạn này.

### 3.44. Pilot Action Class Pattern — `modules/Page/Actions/*` — v0.0.58 (PHASE 6: Release Preparation & Technical Debt Clearance)

**Bối cảnh**: từ CMS-045 tới SEO Admin UI, mỗi lần phát sinh trùng lặp logic Admin↔JSON API (Login/Dashboard/User/Role/Page/Media/Menu/Seo — tổng ~25 cặp Controller), quyết định lặp lại **≥7 lần** là "trùng lặp có chủ đích, không Service/Repository, đúng YAGNI" — vì mỗi cặp chỉ có đúng 2 điểm tiêu thụ, không đạt tiêu chí đã tự đặt ra khi tạo `SiteSettingsManager` (mục 3.42: "dùng bởi ≥4 điểm độc lập"). Phase 6 đánh giá lại quyết định này qua Architecture Analysis — Owner chọn **pilot thí điểm trên 1 nghiệp vụ (`Page`)** trước khi cân nhắc nhân rộng, **không** refactor toàn bộ 25 cặp cùng lúc (rủi ro regression cao nhất từng có — lần đầu *sửa lại* Controller cũ trên diện rộng thay vì chỉ *thêm* Controller mới như mọi Phase trước).

**Kiến trúc**: 1 class `Action` thuần PHP/1 nghiệp vụ (`CreatePageAction`, `UpdatePageAction`, `DeletePageAction`, `PublishPageAction`, `SetHomepageAction` — `modules/Page/Actions/`), chứa **toàn bộ** validate + thao tác DB đã trùng lặp trước đó. Action **không biết gì về HTTP** — không `Request`/`Response`, không `Authorization::can()` (permission check vẫn ở tầng Controller, đúng nguyên tắc "check quyền trước khi vào nghiệp vụ"). Action ném exception cụ thể khi thất bại, không trả `Response`.

**Exception hierarchy** (đúng convention đã có từ `Core\Database\{DatabaseException,QueryException}` — mục 3.3): `PageActionException extends \RuntimeException` (base, không ném trực tiếp) → `PageNotFoundException`, `PageValidationException` (mang `errors(): array<string, list<string>>`) là 2 subclass cụ thể. Controller `catch` đúng subclass và tự quyết định định dạng Response (JSON API: `Response::json([...], 404|422)`; Admin: `Response::html('404 Not Found', 404)` hoặc render lại form với `$exception->errors()`).

**5 Controller JSON API** (`modules/Page/{Create,Edit,Delete,Publish,SetHomepage}PageController.php`) và **5 Controller Admin** (`modules/Admin/Page{Create,Update,Delete,Publish,SetHomepage}Controller.php`) đều inject Action tương ứng qua Constructor Injection (Container tự auto-wire, không cần đăng ký thủ công — Action chỉ phụ thuộc `Database`/`TenantManager`/`Validator`, đều đã có binding sẵn). `ListPagesController`/`PageListController` (List, thuần đọc) và `PageShowCreateController`/`PageShowEditController` (hiển thị form) **không** đưa vào Action — chỉ 1-2 dòng query, trùng lặp không đáng kể, không phải nợ kỹ thuật thật.

**2 điểm khác biệt hành vi được hợp nhất có kiểm chứng** (đã xác nhận qua đọc trực tiếp `tests/Core/ModulePageIntegrationTest.php`/`tests/Core/AdminPageManagementUiTest.php` — không test nào assert vào các nhánh này, an toàn để thống nhất):
1. **`template` rỗng (`''`) khi Create**: JSON gốc chỉ check `!== null` (lưu `''` làm giá trị), Admin gốc check thêm `!== ''` (lưu `null`). Thống nhất theo hành vi Admin (chặt hơn, đúng ý nghĩa nghiệp vụ hơn) — áp dụng cho cả 2 consumer.
2. **`parent_id` rỗng (`''`) khi Create**: cùng discrepancy, cùng cách xử lý.
3. **Cấu trúc `errors` cho lỗi "slug trùng"/"parent không hợp lệ"**: JSON gốc trả `errors: []` (rỗng), Admin gốc trả `errors: {field: [...]}` (đầy đủ, phục vụ hiển thị form). Thống nhất theo Admin (đầy đủ hơn) cho cả 2 — JSON API giờ trả thông tin lỗi chi tiết hơn trước (không phải breaking change vì không test nào assert `errors` rỗng).

**Kết quả pilot**: đã xác nhận qua PHPUnit thật (`ModulePageIntegrationTest`: 17 test/27 assertion, `AdminPageManagementUiTest`: 11 test/24 assertion, full suite 636/636) — không regression. Quyết định có nhân rộng Action Class Pattern ra 24 cặp Controller còn lại hay không **vẫn chưa chốt** — hoãn tới khi Owner đánh giá qua thời gian sử dụng thực tế, **chưa nhân rộng qua Phase 7**.

### 3.45. UI/UX Demo Polish & Enterprise Showcase Pack — v0.0.59 (PHASE 7)

**Bối cảnh**: đóng gói sản phẩm sẵn sàng trình diễn khách hàng/nhà đầu tư — Public Landing Page chuyên nghiệp, dữ liệu demo thật (không còn text kỹ thuật), Admin Dashboard trực quan hơn. Nguyên tắc khóa cứng: **không migration**, **không thêm dependency bên thứ ba**, giữ nguyên CSS3 thuần "Tech Green/Dark".

**`bin/add_site.php` (mới) — giải quyết giới hạn "1 lần" của `bin/bootstrap.php`**: `bin/bootstrap.php` có guard `SELECT COUNT(*) FROM users` — từ chối chạy nếu đã có user, nên không thể dùng lại để tạo Tenant thứ 2. Vì Role Model đã xác nhận từ CMS-037 (`roles.tenant_id IS NULL` = System Role dùng chung mọi tenant), `bin/add_site.php` **không tạo Admin/Role/Permission mới** — chỉ `INSERT sites` + `INSERT site_domains` + `INSERT user_site_roles` (liên kết Admin đã có với `role_id` System Admin có sẵn vào Site mới), bọc trong `Database::transaction()`. Không sửa `bin/bootstrap.php` (đã test, giữ nguyên).

**Enterprise Seeder** (`bin/seed_demo.php`, viết lại hoàn toàn — Fork B1: mở rộng script procedural có sẵn, không tạo pattern Seeder Class mới `database/seeders/`): nhận tham số `[domain] [pack]` (`pack` ∈ `tech`|`restaurant`, mặc định `tech`; `domain` mặc định site đầu tiên — tương thích ngược 100% với cách gọi cũ không tham số từ `SETUP_LOCAL.md`). Mỗi content pack là 1 hàm thuần PHP trả về mảng HTML đã viết sẵn (Hero/Feature Grid/Showcase/CTA cho trang chủ, About/Contact/1 bài blog thật) — **không** schema mới, nội dung lưu đúng quy ước `content['html']` đã có từ Pages Admin UI (mục 3.38).

**Activity Stream — UNION 3 nguồn với ràng buộc dữ liệu thật** (`modules/Admin/DashboardController.php::fetchActivity()`): `SELECT ... FROM pages UNION ALL SELECT ... FROM media UNION ALL SELECT ... FROM users INNER JOIN user_site_roles ...`, sắp xếp `ORDER BY event_at DESC LIMIT 8`. **Không gồm Menu** (Fork C1, Owner Decision) — `menus`/`menu_items` không có cột timestamp nào trong schema thật, đưa vào sẽ cần migration mới, vượt phạm vi "không migration" đã khóa. `pages` dùng `COALESCE(updated_at, created_at)` (vì `updated_at` chỉ set khi có `UPDATE`, `INSERT` không tự điền).

**`themes/default/views/pages/default.php` — sửa xung đột heading**: khi nội dung dùng `content['html']` (Landing Page tự chứa `<h1>` riêng trong Hero Section), **không** render thêm `<h1>{title}</h1>` chung của template (tránh 2 thẻ `<h1>` trùng lặp — sai SEO). Trang `content['text']`/mảng generic/scalar giữ nguyên hành vi cũ (vẫn có `<h1>{title}</h1>` chung, vì nội dung dạng này không tự chứa heading).

**Test dùng theme thật thay vì fixture**: `tests/Core/PublicLandingPageTest.php` (mới) bind `View` vào `themes/default/` thật (không phải `tests/Fixtures/themes/test-theme/` như `PublicPageRenderingTest.php`) — lý do: fixture theme tối giản hóa, chỉ `json_encode()` toàn bộ `$content` thay vì phân nhánh `html`/`text`/array/scalar như theme thật, nên không thể kiểm chứng đúng hành vi `$this->raw()` với HTML phức tạp nhiều section.

**2 lần Root Cause Analysis sau PHPUnit thật**:
1. Tự phát hiện trước khi giao Owner test: `tests/Core/AdminUiFoundationTest.php::migrate()` thiếu bảng `pages`/`media` — `DashboardController` giờ luôn query 2 bảng này.
2. **Bác bỏ RCA sai từ Owner lần thứ 2 trong dự án** (lần đầu: `PublicSearchTest`, mục 3.43): Owner đề xuất đổi `DashboardController.php` sang `users.registered_at AS created_at`. Xác minh trực tiếp migration thật (`2026_08_01_000003_create_users_table.php:21`) xác nhận cột `users.created_at` **tồn tại thật**, không có cột `registered_at` nào trong toàn bộ codebase — áp dụng đề xuất sẽ khiến code tham chiếu cột không tồn tại, lỗi thật trên MySQL/MariaDB production. Nguyên nhân thật: bảng `users` tự tạo riêng trong `AdminUiFoundationTest.php::migrate()` thiếu cột `created_at`. Sửa đúng 1 dòng trong test fixture, **không đụng `DashboardController.php`**.

**Permissions**: không permission mới. **Không migration mới** — đúng cam kết Phase 7.

### 3.46. Beta Release Readiness & Staging Walkthrough — v0.1.0-beta (PHASE 8)

**Bối cảnh**: đóng gói sản phẩm sẵn sàng trình diễn khách hàng doanh nghiệp/nhà đầu tư và triển khai lên Staging thật — **không sửa code PHP nào**, chỉ 2 tài liệu vận hành mới.

**`DEMO_WALKTHROUGH.md`**: kịch bản Demo 4 bước cho Sales/Founder, toàn bộ URL/route dùng trong kịch bản đã xác nhận tồn tại thật qua các Phase trước (không suy đoán) — Landing Page (mục 3.45), Multi-Tenant Switch (`bin/add_site.php` mục 3.45), Admin Dashboard (mục 3.45), Menu Builder kéo-thả AJAX (mục 3.40) + SEO OG/JSON-LD (mục 3.37) + Sitemap/Robots tự sinh (mục 3.42).

**`STAGING_CHECKLIST.md`**: checklist thao tác triển khai Staging/VPS, tham chiếu ngược `DEPLOYMENT.md` (mục 3.44 phạm vi lân cận) thay vì lặp lại nội dung. 2 phát hiện kỹ thuật quan trọng trước khi viết:
1. **`public/uploads/`** (tồn tại vật lý, còn sót trong `.gitignore` cũ) **không phải** nơi lưu Media thật — `UploadMediaController` chỉ ghi vào `storage/app/media/` (xác nhận từ Media Manager Admin UI, mục 3.39). Checklist chỉ liệt kê phân quyền cho đường dẫn đúng, tránh lan truyền nhầm lẫn.
2. **SSL dùng chứng chỉ SAN đa-domain qua `certbot -d ... -d ...`, không phải Wildcard Certificate** — `TenantResolverMiddleware` (mục 3.15 hoặc tương đương) khớp domain chính xác tuyệt đối, các domain tenant là domain độc lập hoàn toàn (không phải subdomain cùng 1 gốc như `cms.test`/`restaurant.test` đã minh chứng ở mục 3.45) — Wildcard Certificate về bản chất không bảo vệ được domain khác gốc.

**Permissions**: không permission mới. **Không migration, không sửa file PHP nào** — đúng cam kết Phase 8.
